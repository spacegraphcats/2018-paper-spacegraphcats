...
graph $G$.  In the following we write $\maxindeg(\dir G)$ to denote the maximum in-degree of a directed graph $\dir G$ and we write
$N^-(v)$ to denote the in-neighbourhood of $v$ in $\dir G$.

First, the graph $\dir G_1$ is a \emph{min-degree
...
\begin{theorem}[\cf Theorem 16 in~\cite{felixThesis}]
