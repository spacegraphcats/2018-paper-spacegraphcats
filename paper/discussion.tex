...
Our approach consequently provides a data exploration framework not available in any other automated software packages.
...
into \pieces. Being able to do this in linear time also ensures that the
indexing and querying can scale to very large data sets. Furthermore, because we
utilize a broad structural characterization (bounded expansion)  rather than a
highly specialized aspect of cDBGs, our methods enable neighborhood-based
...
We note that when we bioinformatically analyze the
...
analysis. The \sgc workflow does not separate regions of the graph shared
