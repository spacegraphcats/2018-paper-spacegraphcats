...
We design and implement~\cite{spacegraphcats} a set of algorithms for efficiently
...
sequencing data (\autoref{fig:domset}). To accomplish this, we first organize the cDBG into {\em \pieces}
...
We selected a threshold (see Appendix) that produces $r$-dominating sets of
...
We first measure how well an inexact query can recover a target genome
from a metagenome. For a benchmark data set, we use the
...
strain variants,
we choose three target genomes from \podarv -- {\em Porphyromonas
  gingivalis ATCC 33277}, {\em Treponema denticola ATCC 35405}, and
{\em Bacteroides thetaiotaomicron VPI-5482} -- that have many
taxonomically close relatives in Genbank. We then use these relatives
...
small number of shared k-mers suffice to define a much larger neighborhood
that contains related genomes.

We next apply neighborhood query to retrieve an unknown genome from \podarv.
...
marker-based analysis with CheckM estimates that 98.8\% of the {\em
  ...
  Proteiniclasticum} variant is recovered. We therefore try using
neighborhood queries to expand our knowledge of the {\em Proteiniclasticum} variant.

We perform a neighborhood query into \podarv with all three known {\em
  Proteiniclasticum} genomes from GenBank.  We then extract the reads
overlapping this neighborhood and assemble them with MEGAHIT.  The
...
the query neighborhood assemble into a 3.1 Mbp genome bin. The
...
neighborhood query retrieves a full draft genome sequence (see Appendix~\ref{subsec:aai}).
...
\begin{figure*}[th]
  ...
  we query \podarv and \hu using each of 64 and 23 genomes fully present
  in the respective datasets and measure the relative size of its neighborhood—a
  ...
  than the original genome bins. We query \hu using each of 23 genomes binned
  from SB1, and assemble the resulting neighborhoods using MEGAHIT and \plass.
  The yellow points represent completeness estimates of MEGAHIT-assembled
...
metagenome differ from mock metagenomes.  We examine genomes inferred
...
in GenBank originating from the SB1 sample, we compare the \hu
neighborhood size distribution with the \podarv data set
(\autoref{fig:hu_strain}(a)). We see that more genome bins in \hu
...
We next investigate the metagenomic content in the query
neighborhoods.  As with the unknown variants in \podarv, we use
...
assemble reads from the query neighborhoods using MEGAHIT.  However,
we find little improvement in the completion metrics (\autoref{fig:hu_strain}(b)).

Investigating further, we find that the query neighborhood assemblies
...
To attempt to recover more gene content from the assemblies, we turn to
...
When we apply \plass to the reads from the query neighborhoods, we see
...
\begin{figure*}[!h]
    ...
    originating from \hu, we find the unbinned content by removing all
...
phylogenetic marker.  In the GenBank bins, we find that 15 of the 23
bins contain at least one gyrA sequence (with 18 {\em gyrA} genes
total).  We therefore use {\em gyrA} for an initial analysis of the
...
cost of sensitivity, we focus only on high-abundance data:
we truncate all reads
in the query neighborhoods at any k-mer that appears fewer than five
times, and run \plass on these abundance-trimmed reads from each
neighborhood.  We then search the gene assemblies with a gyrA-derived
HMM, align all high-scoring matches, and calculate a pairwise similarity
...
we see that ten of the 23 neighborhoods increase in the number of gyrA
...
We see this same pattern for many genes, including alaS, gyrB, rpb2
domain 6, recA, rplB, and rpsC (\appendixref{subsec:othergenes}).
...
identify many novel genes in the query neighborhoods. We use KEGG to
annotate the \plass-assembled amino acid sequences, and subtract any
homolog already annotated in the genome bin.
We also ignore homolog abundance so that each homolog is counted only
